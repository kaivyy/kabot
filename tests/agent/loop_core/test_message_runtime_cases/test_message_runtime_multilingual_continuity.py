...
async def test_process_message_project_inspection_turn_injects_grounded_filesystem_note():
    ...
    assert "config, or workspace docs imply" in current_message
...
@pytest.mark.asyncio
async def test_process_message_config_or_docs_question_uses_grounded_filesystem_note():
    context_builder = MagicMock()
    context_builder.build_messages.return_value = [{"role": "user", "content": "ctx"}]
    session = SimpleNamespace(
        metadata={
            "working_directory": r"C:\Users\Arvy Kairi\.kabot",
            "delivery_route": {
                "channel": "telegram",
                "chat_id": "chat-1",
            },
    ...
    msg = InboundMessage(
        ...
        sender_id="u1",
        chat_id="chat-1",
        content="periksa config dan docs lokalnya, model embedding sudah disetting belum",
    )
    ...
    assert "[System Note: Grounded filesystem inspection]" in current_message
    assert "config, or workspace docs imply" in current_message
    assert "AGENTS.md, config.json" in current_message
