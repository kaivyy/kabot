...
def test_strict_preset_promotes_ask_to_allowlist(tmp_path):
    config_path = tmp_path / "command_approvals.yaml"
    with open(config_path, "w", encoding="utf-8") as f:
        yaml.dump(
            {
    ...
    firewall = CommandFirewall(config_path, preset="strict")
    assert firewall.check_command("git status") == ApprovalDecision.ALLOW
    assert firewall.check_command("unknown command") == ApprovalDecision.ASK
