...
async def test_weather_tool_normalizes_noisy_location_before_fetch(monkeypatch):
    ...
    wttr_mock.assert_awaited_once_with("Cilacap", "simple")


@pytest.mark.asyncio
async def test_weather_tool_localizes_missing_location_message():
    tool = WeatherTool()
    query = "   "
    result = await tool.execute(location=query)
    assert result == i18n_t("weather.need_location", query)


@pytest.mark.asyncio
async def test_weather_tool_localizes_fetch_failure_message(monkeypatch):
    ...
    tool = WeatherTool()
    query = "suhu di cilacap sekarang"
    result = await tool.execute(location=query, context_text=query)

    assert result == i18n_t("weather.fetch_failed", query, location="Cilacap")
...
async def test_weather_tool_normalizes_degree_phrasing(monkeypatch):
    ...
    tool = WeatherTool()
    result = await tool.execute(location="purwokerto berapa derajat sekarang")
