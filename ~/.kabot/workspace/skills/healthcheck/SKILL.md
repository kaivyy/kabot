---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Kabot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Kabot cron scheduling for periodic checks, or version status checks on a machine running Kabot (laptop, workstation, Pi, VPS).
---

# Kabot Host Hardening

## Overview

Assess and harden the host running Kabot, then align it to a user-defined risk tolerance without breaking access. Use Kabot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Kabot changes the host firewall, SSH, or OS updates; it does not.
...
5. Kabot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Kabot can access sensitive data.
...
### 2) Run Kabot security audits (read-only)

As part of the default read-only checks, run `kabot security audit --deep`. Only offer alternatives if the user requests them:

1. `kabot security audit` (faster, non-probing)
2. `kabot security audit --json` (structured output)

Offer to apply Kabot safe defaults (numbered):

1. `kabot security audit --fix`

Be explicit that `--fix` only tightens Kabot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Kabot version/update status (read-only)

As part of the default read-only checks, run `kabot update status`.
...
- Credential hygiene notes (location of Kabot creds, prefer disk encryption)
...
After Kabot install or first hardening pass, run at least one baseline audit and version check:

- `kabot security audit`
- `kabot security audit --deep`
- `kabot update status`

Ongoing monitoring is recommended. Use the Kabot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. â€œDo you want me to schedule periodic audits (e.g., daily/weekly) via `kabot cron add`?â€

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `kabot update status`
...
Before creating, `kabot cron list` and match on exact `name`. If found, `kabot cron edit <id> ...`.
If not found, `kabot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `kabot update status` (preferred for source checkouts and channels)
2. `npm view kabot version` (published npm version)

## Kabot command accuracy

Use only supported commands and flags:

- `kabot security audit [--deep] [--fix] [--json]`
- `kabot status` / `kabot status --deep`
- `kabot health --json`
- `kabot update status`
- `kabot cron add|list|runs|run`

Do not invent CLI flags or imply Kabot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Kabot compaction:
