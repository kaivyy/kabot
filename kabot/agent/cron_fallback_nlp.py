...
def score_required_tool_intents(
    ...
    *,
    ...
    # 3) Weather.
    has_weather_marker = _contains_any(q_lower, WEATHER_KEYWORDS, fuzzy_latin=True)
    if has_weather_marker:
        add("weather", 0.64, "weather-lexicon")
    location_candidate = extract_weather_location(text)
    if has_weather_marker and location_candidate:
    ...
    if _contains_any(q_lower, SERVER_MONITOR_KEYWORDS, fuzzy_latin=True):
        add("server_monitor", 0.82, "server-monitor-lexicon")
    if _contains_any(q_lower, PROCESS_RAM_KEYWORDS, fuzzy_latin=True):
        add("get_process_memory", 0.76, "process-memory-lexicon")
    if _contains_any(q_lower, _RAM_CAPACITY_MARKERS, fuzzy_latin=True):
        add("get_system_info", 0.82, "ram-capacity-structure")
    if _contains_any(q_lower, SYSTEM_INFO_KEYWORDS, fuzzy_latin=True) or _contains_any(
    ...
    has_stock_marker = _contains_any(q_lower, STOCK_KEYWORDS, fuzzy_latin=True)
    ...
    if has_stock_marker:
        add("stock", 0.56, "stock-lexicon")
    if stock_name_candidates and not stock_symbols:
        ...
        if has_value_query_marker and not has_personal_chat_marker and not has_conflicting_domain:
    ...
    # 7) Speedtest / cleanup.
    if _contains_any(q_lower, SPEEDTEST_KEYWORDS, fuzzy_latin=True):
        add("speedtest", 0.8, "speedtest-lexicon")
    if _contains_any(q_lower, CLEANUP_KEYWORDS, fuzzy_latin=True) and _contains_any(
        q_lower, CLEANUP_ACTION_KEYWORDS, fuzzy_latin=True
    ):
        add("cleanup_system", 0.86, "cleanup-action")
    elif _contains_any(q_lower, CLEANUP_KEYWORDS, fuzzy_latin=True):
...
def extract_weather_location(question: str) -> str | None:
    ...
    def _strip_weather_terms(value: str) -> str:
        ...
        for term in sorted(set(WEATHER_KEYWORDS), key=len, reverse=True):
